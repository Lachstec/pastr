...
CREATE FUNCTION users_cofirmations_delete_expired() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    DELETE FROM pastr.users_confirmations WHERE created_at < NOW() - INTERVAL '2 days';
    RETURN NEW;
END;
$$;

CREATE TRIGGER users_confirmations_delete_expired_trigger
    AFTER INSERT ON pastr.users_confirmations
    EXECUTE PROCEDURE users_cofirmations_delete_expired();